# 图片管理系统

一个功能强大的基于 Next.js 14 的全栈图片管理系统，支持 AI 智能问答、地图视图、图片编辑等功能。

## ✨ 主要功能特性

### 🖼️ 核心功能
- **图片上传与管理** - 支持拖拽上传，自动生成缩略图，保留 EXIF 数据
- **图片画廊** - 响应式网格展示，支持分页和无限滚动
- **智能搜索** - 支持按标签、时间、位置等多维度搜索
- **图片编辑** - 裁剪、滤镜、旋转等基础编辑功能
- **标签管理** - 自动标签生成和手动标签管理

### 🤖 AI 智能功能
- **AI 图片问答** - 在图片详情页直接与 AI 对话，了解图片内容
- **自动标签生成** - 基于图片内容自动生成标签和描述
- **智能分类** - AI 自动识别场景、物体、人物等

### 🗺️ 地图视图
- **地理位置展示** - 在地图上查看所有带 GPS 的照片位置
- **位置筛选** - 按地理位置筛选和搜索照片
- **旅行记录** - 追溯拍摄轨迹和地点

### 📁 组织与管理
- **相册管理** - 创建和管理个人相册
- **收藏功能** - 快速收藏喜欢的照片
- **回收站** - 误删恢复机制

### 🔐 用户系统
- **用户认证** - 基于 NextAuth.js 的安全认证
- **权限控制** - 每个用户只能访问自己的图片
- **多用户支持** - 支持多用户同时使用

## 🛠️ 技术栈

### 前端技术
- **框架**: Next.js 14.2.18 (React 18.3.1)
- **语言**: TypeScript 5.6.3
- **样式**: Tailwind CSS 3.4.18 + shadcn/ui
- **图标**: Lucide React
- **状态管理**: Zustand
- **表单管理**: React Hook Form + Zod 验证

### 后端技术
- **运行时**: Node.js
- **API**: Next.js API Routes
- **ORM**: Prisma 5.22.0
- **数据库**: MySQL 8.0+
- **认证**: NextAuth.js 4.24.10

### 图片处理
- **图像处理**: Sharp (高性能图像处理)
- **EXIF 解析**: exifr (提取照片元数据)
- **图像编辑**: CropperJS

### AI 服务
- **多模型支持**: DeepSeek、百度 AI、豆包等
- **图片理解**: 支持 Vision 模型
- **文本生成**: 多轮对话支持

### 地图服务
- **地图库**: Leaflet + React-Leaflet
- **地图瓦片**: OpenStreetMap (免费)

## 🚀 快速开始

### 前置要求
- Node.js 18+
- MySQL 8.0+
- npm 或 yarn

### 1. 克隆项目

```bash
git clone [<repository-url>](https://github.com/BruceY-rgb/PictureManagementSystem.git)
cd PictureManagementSystem
```

### 2. 安装依赖

```bash
npm install
```

### 3. 配置环境变量

复制环境变量文件并配置：

```bash
cp .env.example .env
```

编辑 `.env` 文件：

```env
# 数据库配置
DATABASE_URL="mysql://root:password@localhost:3306/picture_management"

# NextAuth 配置
NEXTAUTH_SECRET="your-secret-key-here"
# 使用以下命令生成密钥:
# openssl rand -base64 32

NEXTAUTH_URL="http://localhost:3000"

# AI 服务配置 (可选)
# DeepSeek (用于 AI 问答)
DEEPSEEK_API_KEY="your-deepseek-api-key"
DEEPSEEK_BASE_URL="https://api.deepseek.com"

# 百度 AI (用于图像识别)
BAIDU_API_KEY="your-baidu-api-key"
BAIDU_SECRET_KEY="your-baidu-secret-key"

# 豆包 AI (备选方案)
DOUBAO_API_KEY="your-doubao-api-key"
DOUBAO_BASE_URL="https://ark.cn-beijing.volces.com/api/v3"
DOUBAO_MODEL="doubao-1.5-vision-pro-250328"
```

### 4. 初始化数据库

```bash
# 创建数据库迁移
npx prisma migrate dev --name init

# 生成 Prisma Client
npx prisma generate

# (可选) 查看数据库
npx prisma studio
```

### 5. 启动开发服务器

```bash
npm run dev
```

打开 [http://localhost:3000](http://localhost:3000)

## 📁 项目结构

```
src/
├── app/                    # Next.js App Router
│   ├── api/               # API 路由
│   │   ├── auth/          # 认证 API
│   │   ├── images/        # 图片相关 API
│   │   ├── search/        # 搜索 API
│   │   └── tags/          # 标签 API
│   ├── gallery/           # 图片画廊页面
│   │   ├── page.tsx       # 所有图片
│   │   └── [id]/page.tsx  # 图片详情页
│   ├── map/               # 地图视图页面
│   ├── albums/            # 相册页面
│   ├── favorites/         # 收藏页面
│   ├── trash/             # 回收站
│   ├── search/            # 搜索页面
│   ├── upload/            # 上传页面
│   ├── login/             # 登录页面
│   └── register/          # 注册页面
├── components/            # 可复用组件
│   ├── ui/               # shadcn/ui 基础组件
│   ├── image/            # 图片相关组件
│   ├── layout/           # 布局组件
│   ├── search/           # 搜索相关组件
│   └── tags/             # 标签相关组件
├── lib/                   # 工具函数库
│   ├── ai/               # AI 服务封装
│   │   ├── deepseek.ts   # DeepSeek API
│   │   ├── doubao.ts     # 豆包 API
│   │   └── baidu.ts      # 百度 AI
│   ├── auth.ts           # NextAuth 配置
│   ├── prisma.ts         # Prisma 客户端
│   ├── image-utils.ts    # 图片处理工具
│   └── utils.ts          # 通用工具函数
├── types/                 # TypeScript 类型定义
│   ├── image.ts          # 图片相关类型
│   ├── user.ts           # 用户相关类型
│   └── api.ts            # API 相关类型
├── hooks/                 # 自定义 React Hooks
└── stores/                # Zustand 状态管理

prisma/
├── schema.prisma          # 数据库 Schema
└── migrations/            # 数据库迁移文件
```

## 📚 主要功能说明

### 图片上传
- 支持拖拽上传或点击上传
- 自动提取 EXIF 数据（拍摄时间、相机信息、GPS 等）
- 生成多种尺寸缩略图（150x150, 400x400, 800x800）
- 支持 JPG、PNG、GIF、WebP 格式

### 图片画廊
- 响应式网格布局
- 支持分页和无限滚动
- 实时搜索和筛选
- 快捷操作：收藏、删除、编辑

### AI 图片问答
- 位于图片详情页右侧
- 支持多轮对话
- 基于图片 EXIF 数据和 AI 分析结果回答
- 快捷问题建议

### 地图视图
- 显示所有带 GPS 的照片位置
- 点击标记查看照片预览
- 自动适应所有标记位置
- 统计信息（有位置/无位置照片数量）

### 搜索功能
- 关键词搜索
- 按标签筛选
- 按时间范围筛选
- 按位置筛选（经纬度范围）
- 按收藏状态筛选

## 🔌 API 接口

### 图片相关
- `GET /api/images` - 获取图片列表
- `POST /api/images/upload` - 上传图片
- `GET /api/images/[id]` - 获取图片详情
- `PATCH /api/images/[id]` - 更新图片信息
- `DELETE /api/images/[id]` - 删除图片
- `GET /api/images/[id]/file` - 获取图片文件
- `POST /api/images/[id]/tags` - 添加标签
- `DELETE /api/images/[id]/tags` - 删除标签
- `GET /api/images/locations` - 获取所有图片位置（地图用）
- `POST /api/images/[id]/chat` - AI 问答

### 搜索
- `GET /api/images/search` - 搜索图片
- `GET /api/search/nl` - 自然语言搜索


## 🧪 可用脚本

```bash
# 开发
npm run dev          # 启动开发服务器
npm run build        # 构建生产版本
npm run start        # 启动生产服务器

# 代码质量
npm run lint         # 运行 ESLint
npm run lint:fix     # 自动修复 ESLint 错误

# 数据库
npx prisma migrate dev        # 创建并应用迁移
npx prisma generate           # 生成 Prisma Client
npx prisma studio             # 打开数据库管理界面
npx prisma db seed            # 运行种子数据

# 类型检查
npx tsc --noEmit              # TypeScript 类型检查
```

## 🌐 部署

### Vercel 部署 (推荐)

1. Fork 项目到 GitHub
2. 在 Vercel 中导入项目
3. 配置环境变量
4. 部署

### Docker 部署

```bash
# 构建镜像
docker build -t picture-management .

# 运行容器
docker run -p 3000:3000 --env-file .env picture-management
```

### 手动部署

1. 构建项目：
```bash
npm run build
```

2. 启动生产服务器：
```bash
npm run start
```

确保生产环境已配置：
- MySQL 数据库
- 所有环境变量
- HTTPS（生产环境）

## 🤝 贡献指南

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

### 开发规范
- 使用 TypeScript 进行类型安全开发
- 遵循 ESLint 配置
- 组件使用函数式组件 + Hooks
- 提交信息使用约定式提交格式

## 📄 许可证

本项目为浙江大学课程作业项目，遵循 MIT 许可证。详见 [LICENSE](LICENSE) 文件。

## 🙏 致谢

- [Next.js](https://nextjs.org/) - React 框架
- [Prisma](https://www.prisma.io/) - 数据库 ORM
- [Tailwind CSS](https://tailwindcss.com/) - CSS 框架
- [shadcn/ui](https://ui.shadcn.com/) - UI 组件库
- [Sharp](https://sharp.pixelplumbing.com/) - 图像处理
- [Leaflet](https://leafletjs.com/) - 地图库

## 📞 联系我们

如有问题或建议，请提交 Issue 或联系项目维护者。

---

⭐ 如果这个项目对你有帮助，请给它一个星标！
